...
//        die("<i class='alert-danger glyphicon glyphicon-danger'></i> No file selected, Please select the file to upload");
    ...
    //ADD TO DATABASE
    $sql="INSERT INTO profile_pic(name,profileimage)VALUES('{$userfirstname}','{$path}')";

    $query=mysqli_query($connection,$sql);
    if ($query) {
        echo "<div class='alert alert-success'>
            <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
            <strong>Profile Upload!</strong>  Profile Pic uploaded successfully.
            </div> ";
...
<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
            <strong>Profile Upload Error!</strong> Profile not uploaded.Please Trv Again..
</div> ";
    }
